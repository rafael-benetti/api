            ...
            for (const typeMachine of machines) {
                ...
                const boxes = boxesIds.map(boxeId => {
                    ...
                    const counters = typeCounters.map(typeCounter => {
                        ...
                        if (typeCounter.name.toLowerCase().includes('entrada') ||
                            typeCounter.name.toLowerCase().includes('coin') ||
                            typeCounter.name.toLowerCase().includes('box') ||
                            typeCounter.name.toLowerCase().includes('0') || // ANCHOR DESCOBRIR QUE TIPO É
                            typeCounter.name.toLowerCase().includes('haste') // ANCHOR SABER QUE TIPO É
                        )
                ...
                const locationId = (await this.client.get(`@points:${typeMachine.sellingPointId}`));
