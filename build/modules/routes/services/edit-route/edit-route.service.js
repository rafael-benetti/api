...
let EditRouteService = class EditRouteService {
    constructor(routesRepository, usersRepository, groupsRepository, machinesRepository, pointsOfSaleRepository, logsRepository, ormProvider) {
    ...
    async execute({ userId, routeId, label, operatorId, pointsOfSaleIds, }) {
        ...
        if (pointsOfSaleIds) {
            ...
            const newPos = pointsOfSale.filter(newPos => !route.pointsOfSaleIds.includes(newPos.id));
            newPos.forEach(pos => {
                this.logsRepository.create({
                    createdBy: user.id,
                    ownerId: user.ownerId || user.id,
                    type: log_type_enum_1.default.ADD_POS_TO_ROUTE,
                    posId: pos.id,
        ...
        this.logsRepository.create({
            ...
            groupId: undefined,
            ownerId: route.ownerId,
            type: log_type_enum_1.default.EDIT_ROUTE,
            routeId: route.id,
        });
...
EditRouteService = __decorate([
    ...
    __param(5, tsyringe_1.inject('LogsRepository')),
